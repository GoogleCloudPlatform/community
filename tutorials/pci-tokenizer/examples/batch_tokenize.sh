...
# Usage:
# ./batch_tokenize.sh <CSV_FILE_PATH>
# Example values will be provided for any missing input

# Output:
# output file is csv_files/batch_tokenize_sample_csv_output.csv

. $(dirname "$0")/envvars
test -f $(dirname "$0")/local.envvars && . $(dirname "$0")/local.envvars

if [ "$1" = "" ]; then
  CSV_FILE_PATH="csv_files/batch_tokenize_sample_csv.csv"
else
  CSV_FILE_PATH=$1
fi

AUTH=$(gcloud auth print-identity-token)
echo $(python3 batch_tokenize_and_detokenize.py --csv_file_path "$CSV_FILE_PATH" --auth_token "$AUTH" --project_id "$PROJECT" --app_url "$URL/tokenize" --transformation_type "tokenize")
