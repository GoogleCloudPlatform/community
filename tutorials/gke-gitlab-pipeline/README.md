...
1.	Download the code from github at this link.  https://github.com/nlonginow/demo-kubernetes-pipeline
...
3.	Turn on Auto-Devops. This will cause Gitlab to run our pipeline on each checkin to the repository.  In your Gitlab project, go to Settings - CI/CD and expand out the Auto DevOps section.  Check the box to enable.  
...
Next, the Service.  We are deploying a LoadBalancer type service on port 8081, which will provide us with an externally accessible IP.  This is not the least expensive way to access a pod, but for purposes of this demonstration it is the simplest.  Look into an Ingress service for a less expensive approach.  We are giving the port a name ("name:") because we need to reference it in our test scripts.
