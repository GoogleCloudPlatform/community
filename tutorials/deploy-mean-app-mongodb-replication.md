...
1. From the [Cloud Launcher](https://console.cloud.google.com/launcher), select the [`MEAN Certified by Bitnami`](https://console.cloud.google.com/launcher/details/bitnami-launchpad/meanstack) template.
...
1. From the [Cloud Launcher](https://console.cloud.google.com/launcher), select the [`MongoDB with Replication`](https://console.cloud.google.com/launcher/details/bitnami-launchpad/mongodb-multivm) template.
1. Review the information and cost. Click `Launch on Compute Engine` to proceed.
...
Browse to the public IP address of your Bitnami MEAN instance and confirm that it is working as expected. Note that since the MEAN application is now connected to a different MongoDB database, it will be necessary to re-register a user account before logging in to the example application.

## Test service continuity (optional)

In the Bitnami MongoDB with Replication template, one node in the cluster is designated as the primary node and receives all write operations, while other nodes are designated as secondary nodes which hold their own copies of the data set. In case the primary node fails, the secondary nodes and arbiters will elect a new primary automatically to ensure that writes continue without interruption. This provides redundancy and ensures minimal downtime for your application.
