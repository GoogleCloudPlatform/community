...
1.  Go to the [Cloud SQL Instances page](https://console.cloud.google.com/sql/)
		of Cloud Console. Click **Create Instance**.
1.  Choose `MySQL` as the database engine and `Second Generation` as the
		instance type.
...
1.  Choose the [location of your Cloud SQL instance](https://cloud.google.com/compute/docs/regions-zones/).
		...
		By default Cloud SQL creates an instance using one CPU core, 3.75GB memory,
		and 10GB SSD storage. Depending on the scale of your database backend, you
...
1.  In the Instance Details page, select the **Authorization** tab. Under
...
1.  Go to the [Cloud SQL Instances page](https://console.cloud.google.com/sql/instances)
		and click the ID of the instance you created. In the Instance Details page,
...
1.  Declare rules with the following SQL queries:
		...
		- Distribute all the `SELECT … FOR UPDATE` queries to database servers in
			host group `1`
		- Distribute all the `SELECT` queries to database servers in host group `0`

		Queries that do not match any existing rules go to database servers in host
		group `0` (default).

1.  Add the master instance to host group `1` so that it also helps serve read
		traffic:
...
1.  Adjust the weights of servers in host group `1`:
		...
		Replace `[IP_ADDRESS_1]`, `[IP_ADDRESS_2]`, `[IP_ADDRESS]` with the IP
		addresses of read replica #1, read replica #2, and the master instance
		...
		Read replica #1 has a weight of 40, so its chance of receiving a query is
...
1.  Update the status of the read replica to `OFFLINE_SOFT`:
		...
		status of the read replica in Cloud Console by going to the
		...
		chart is available in the Instance Details page. You should see that your
...
1.  If you do not plan to use the read replica any more in the future, delete it
		from `mysql_servers` table:
		...
		1.  Go to the [Cloud SQL Instances page](https://console.cloud.google.com/sql/instances)
				and click the read replica you would like to remove.

		1.  Click **Delete**. Follow the instructions on screen to delete the
				instance.
