...
Run `terraform apply` followed by `terraform output ip` to return the instance’s external ip address. Validate that everything is set up correctly at this point by ssh’ing to that ip address.
```Shell
