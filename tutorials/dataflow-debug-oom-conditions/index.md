...
This document is a summary of the information you will need to know in order to detect and troubleshoot out-of-memory conditions in workers, either because of bugs or memory leaks, or because they simply need more memory than the system allows them to use.

## The dataflow worker
"Worker" is the name Dataflow gives to GCE instances that act as processing nodes for Dataflow pipelines. 

These machines run: 

* The code that actually transforms data (normally known as "user code"). This is the code that you write, and may also include 3rd-party libraries that your code uses.
* A framework (that Dataflow calls "harness") that wraps the user code and communicates data back and forth between it and Dataflow Components like Shuffle, and your pipeline's sources and sinks.
* A series of monitoring agents that report statistics, progress and worker health to the Dataflow backend. Some of this information is displayed in the pipeline's view in Cloud Console.
* In addition to all the components above, Dataflow pipelines also run a few extra components to store the pipeline's state (like intermediate results and open windows). These components are referred to as "Shuffle" and there are two versions of them:
	* The original implementations, that Dataflow has been using since the launch of Dataflow, runs as a separate process in the GCE workers. 
	* A new implementation, known as Dataflow Shuffle (for batch pipelines) or Streaming Engine (for streaming pipelines) runs in a different set of hosts. These Shuffle hosts are in Google's infrastructure, and are independent of both your project and GCE itself.
...
Inside the JVM the situation is similar: a single memory space (the JVM heap) is used by multiple threads (one processing thread per CPU core, plus many others) to store information. 

If any of the user code needs to use more than its proportional amount of memory (total size of memory divided by number of process threads), the JVM could run out of free space and into an out-of-memory condition.
Even though there are hundreds of threads running in worker JVMs, most use a negligible amount of memory. Unless you need to account for every single KB of memory utilization, you only need to worry about the process threads (as many as CPU cores, by default).

### Signs that you are having OOM issues

Pipelines having OOM issues will often get stuck, or fail with no obvious symptoms (e.g. few, if any, errors logged in the job's view in Cloud Console).
...
The messages "*thrashing=true*" and "*OutOfMemoryError*" come from the JVM and they mean that the Java code is exhausting the JVM's heap. Keep reading to understand how to troubleshoot these errors.

The message "*Out of memory: kill process <Pid> (<process_name>) score <number> or sacrifice child*" means that some other process in the worker is causing the GCE machine itself to run out of RAM. Dataflow workers don't have swap space, so once the RAM is all used up, there is nothing else left.
This message means that the Linux kernel itself has started killing processes that threatened the stability of the entire system. If you see this particular error message, open a support ticket or report it to Google. 
...
If you find that your job is failing due to the JVM running out of memory, you will need to take a heap dump in order to find out what is causing it.
There are three main ways of obtaining a heap dump from JVMs running in Dataflow workers. In order of preference, they are:
 
* By instructing the pipeline's JVMs to automatically dump their heap on OOM.
* By connecting to the worker machine and downloading the heap through a browser.
* By connecting directly to a JVM through JMX.
...
* Just because your Dataflow workers have a certain amount of RAM installed, that doesn't mean that your code can use all that memory. There are many other things going on in the worker machines.
	* If your pipeline **doesn't**  use Dataflow Shuffle (batch) or Streaming Engine (streaming) the heap available to your code is roughly 40% of the total memory.
	* If your pipeline **does** use Dataflow Shuffle or Streaming Engine, the heap available to your code is roughly 80% of the total memory.
...
* Don't cache or buffer data (Dataflow takes care of that for you), and keep as little state in RAM as possible.
* If you run out of memory:
	* Try using highmem workers.
	* Try using fewer threads.
* If everything else fails, obtain a heap dump and track down the source of the OOM.
