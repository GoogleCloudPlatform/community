...
1. Create a Google Cloud Storage bucket and the backend resource for the Terraform state file:
   ...
   You can use Terraform to customize your in-cluster Anthos Service Mesh control plane using `IstioOperator` resource files. In this tutorial, you use a simple `IstioOperator` resource file to customize your`istio-ingressgateways`.
...
For more information about metrics, logs and tracing with Anthos Service Mesh, see [Exploring Anthos Service Mesh in the Cloud Console](https://cloud.google.com/service-mesh/docs/observability/explore-dashboard).
