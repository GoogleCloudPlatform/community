...
*   [iPerf3](https://iperf.fr/): A commonly used network testing tool that can
    ...
    throughput of the network that carries them. **NB: Though readily available,
    this may only be appropriate for single-CPU machines.**
*   [Netperf](https://hewlettpackard.github.io/netperf/): Similar to iPerf3 but
    more appropriate for throughput testing on multii-CPU instances that are
    CPU-bound on a single CPU. Limitations of iperf3 and discussed the the
    'Throughput testing with Netperf' section below
...
## Throughput Testing with Netperf


CPU throttling is likely to be a bottleneck when testing with iperf3. The `-P`
flag launches multiple client streams, but these all run in a single thread.
From iperf3 man pages;
...
Also, any attempt to launch multiple clients will return the following complaint
...
Allowing only a single process to run is
[also by design choice](https://github.com/esnet/iperf/issues/140)


To exercise multiple CPUs, an alternative is to install and use netperf
[http://g3doc/net/debug/netninja/g3doc/netperf.md](http://g3doc/net/debug/netninja/g3doc/netperf.md)


NB: This should be compiled with the `--enable-demo=yes` option to see the
...
Start the client. The invocation below has the format;
...
You may then start multiple clients assigning them to different CPUs. The
example below starts 10 processes and assigns them to the respective CPUs


```
for i in {1..10}
do
taskset -c $i src/netperf -D 2 -H <Server_IP> -f G &
done
```