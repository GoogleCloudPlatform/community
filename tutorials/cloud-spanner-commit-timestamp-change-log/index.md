---
title: Using Cloud Spanner commit timestamp feature to create a change log with Go
description: Learn how to use Cloud Spanner commit timestamp feature to create a change log.
author: jsimonweb
...
which records changed most recently. Using Cloud Spanner's commit timestamp feature makes this easy.
...
when you create your tables. Below is a SQL statement that will create a Spanner table that includes a timestamp column named "Timestamp". The extra
options attribute "OPTIONS(allow_commit_timestamp=true)" makes "Timestamp" a commit timestamp column and enables it to be auto-populated with the exact transaction timestamp for INSERT, UPDATE, and DELETE operations on a given table row.
...
This code updates five records and uses the "spanner.CommitTimestamp" constant which will populate the "Timestamp" column
...
```
Now if you want to find the last five records updated in the table,
...
This query uses an ORDER BY clause on the "Timestamp" commit timestamp column
to sort the result by descending order of their timestamps.

You can run this SQL Statement to query the DocumentsWithTimestamp table from the [**Cloud Spanner**](https://console.cloud.google.com/spanner) Cloud Console:
...
As an alternative to just adding timestamp columns to your tables, if you would like to more throughly track changes to your records over time, you can create a companion History table when you create your tables.

Here are example SQL statements to create two tables, a Documents table and a DocumentHistory table.
...
Here's a snippet of Go code that updates 3 records in each table as a transaction with commit timestamps:
...
Now you can do a query like the following to get the last three records updated in the Documents table
combined with the previous Contents from the most recent change.
...
tables from the [**Cloud Spanner**](https://console.cloud.google.com/spanner) Cloud Console:
...
* If you just want to track when records change in a particular table, create the table with a commit timestamp feature.
* When creating a table for which you want to keep a log of how records change over time, create an additional History table
  that includes a commit timestamp column. Then use transactions to update the History table when records are
  inserted or updated in the table for which changes should be logged.
