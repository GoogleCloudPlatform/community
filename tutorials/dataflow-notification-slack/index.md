...
1.  In Cloud Shell, set the working project by replacing `[PROJECT_ID]` with your project ID:
...
1.  In Cloud Shell, run a new Dataflow job that will fail:
     ...
     Because the `inputFile` value for this job is a non-existent file, the job will fail.
