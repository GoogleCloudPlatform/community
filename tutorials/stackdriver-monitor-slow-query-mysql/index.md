...
Note: If you are using an existing database that already has slow queries visible in the Logs Viewer, you can skip this and proceed directly to the next section.
...
To test the expression, click **Build** to open the Regular Expression Editor, which shows what will be extracted from the log. Matching parts of SQL statements are highlighted in blue, while discarded parts are highlighted in yellow.
...
You have now configured MySQL and Stackdriver Logging for slow query logs. Proceed to [Generate sample data with a load test](#generatesample).
