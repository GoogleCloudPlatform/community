...
def publish(client, device, temp, pres, humi, solar, wind):
    temp, pres, humi, solar, wind = read_sensors(device, temp, pres, humi, solar, wind)
    payload = {}
...
def read_sensors(device, temp, pres, humi, solar, wind):
    """
    Read all the sensors of the Pimoroni Enviro Phat and return floats
    Or simulate the values with a random walk
    """
    try:
        if device == 'pi':
            ...
            solar, wind = read_arduino_sensors()
        else:
...
def read_arduino_sensors():
    s = {}
    response = serial_send_and_receive('0')
...
def serial_send_and_receive(theinput):
    ser.write(theinput)
    while True:
        try:
            sleep(0.01)
...
def simulate_sensors(prev, stdev, min, max):
    ...
    temp: mean: 22 standard deviation: 10 min: -40 max: 50
    pressure: mean: 1013 stdev: 6 min: 850 max: 1150
    humidity: mean: 50 stedv: 20 min: 0 max: 100
    random.gauss(mu, sigma) - Gaussian distribution. mu is the mean, and sigma
...
while True:
    ...
    publish(client, device, temp, pres, humi, solar, wind)
    sleep(1)
