...
author: markku
tags: IoT, Internet of Things, Arduino, Raspberry Pi
date_published: 
---

Markku Lepisto | Solutions Architect | Google Cloud

This two-part tutorial demonstrates how to control an [Arduino Microcontroller][arduino] with a [Raspberry Pi][pi], connect the devices to [Google Cloud IoT Core][iotcore], post sensor data from the devices, and analyze the data in real time.
...
- Post the data to Google Cloud IoT Core over a secure [MQTT][mqtt] connection
...
You will also need to download and install the [Arduino IDE][ide] on your local development environment.
...
## Required hardware
- Raspberry Pi 3 Model B or B+. **Note:** the instructions should work on other Pi models as well, but this has not been verified
...
- Arduino Uno Genuino or 100% compatible microcontroller
- Two (recommended) analog sensors for the Arduino. In this tutorial we use a small solar panel and a small DC motor to generate analog voltage readings
...
1. Select `Sketch > Verify/Compile` from the Arduino IDE menu.
1. Select `Sketch > Upload` to transfer the compiled Sketch to your Arduino over the USB serial connection. You should see the Arduino's LEDs blink rapidly while the binary is transferred over.
1. Select `Tools > Serial Monitor` to open the Serial Monitor window.
1. Test requesting the current sensor values from the Arduino, by typing `0` (zero) in the Serial Monitor's input window, and pressing `<enter>`. If the Sketch and serial connection works, you should see a response similar to: `S s:136 w:43` in the Serial Monitor. Here, `S` indicates sensor output, `s:136` is the current value of the solar panel (analog pin `A1`) and `w:43` is the current value of the wind generator / DC motor (analog pin `A0`). This is how the Python client app on the Pi will request the sensor readings from the Arduino later on. Command `0` is an example of a simple protocol. Sending e.g `1` could be used to command the Arduino to do something else, such as turn a servo motor. In this tutorial, the Arduino Sketch has implementation only for command `0` - get voltage values of analog pins `A0` and `A1`
1. Unplug the USB cable from your local machine, to disconnect and power down the Arduino. The Arduino will now run the sketch automatically, when ever it gets power.
...
1. Connect the Arduino to the Pi with a USB cable. This will power on the Arduino, and create a USB serial port on the Pi, for serial communications.
1. On the Pi, check that you now have a serial port connected to the Pi. Execute: `$ file /dev/ttyACM0`. The output should be similar to: `/dev/ttyACM0: character special (166/0)`
1. With your virtualenv active, run the client app on the Pi, by executing:
```
(venv) $ python solarwind.py  --project_id <your_project_id> --registry_id my-registry --device_id my-device --private_key_file rsa_private.pem --algorithm RS256 --cloud_region us-central1 --ca_certs roots.pem --message_type event --device_type pi
```
