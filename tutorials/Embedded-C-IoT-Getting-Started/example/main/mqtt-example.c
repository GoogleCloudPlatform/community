...
{
    char *publish_topic = NULL;
    asprintf(&publish_topic, PUBLISH_TOPIC_EVENT, "device");
...
void iotc_mqttlogic_attach_subscribe_callback(iotc_context_handle_t in_context_handle, iotc_sub_call_type_t call_type, const iotc_sub_call_params_t *const params, iotc_state_t state, void *user_data)
{
...
{
    asprintf(&subscribe_topic_command, SUBSCRIBE_TOPIC_COMMAND, "device");
    printf("subscribe to topic: \"%s\"\n", subscribe_topic_command);
    iotc_state_t subSt = iotc_subscribe(in_context_handle, "/devices/device/commands/#", 0, iotc_mqttlogic_subscribe_callback, /*user_data=*/NULL);
    delayed_publish_task = iotc_schedule_timed_task(in_context_handle, publish_delegate_telemetry_event, 10, 0, /*user_data=*/NULL);
}

void subscribeConfig(iotc_context_handle_t in_context_handle)
{
    asprintf(&subscribe_topic_config, SUBSCRIBE_TOPIC_CONFIG, "device");
...
{
    iotc_state_t st = iotc_publish(in_context_handle, "/devices/device/attach", "{}", 1, subscribeConfig, /*user_data=*/NULL);
}
